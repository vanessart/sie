...
foreach ($cargo as $cg) {
    unset($escolas);
    unset($usado);
    $dis = sql::get(['dtgp_cadampe_esc', 'instancia'], 'fk_id_inst, m, n, t', ['fk_id_cargo' => @$cg['id_cargo']]);
    foreach ($dis as $v) {
    ...
    $sql = " select n_inst, id_inst, bairro, m, t, n from instancia i "
            ...
            . " left join dtpg_cadamde_esc_vaga cv on cv.fk_id_inst = i.id_inst and cv.fk_id_cargo = " . @$cg['id_cargo'] . ' '
            . " order by n_inst";
