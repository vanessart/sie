<?php
if (tool::id_inst() == 124) {
    $tec = $model->pegarespon(3);
} else {
    $tec = $model->pegarespon(1);
}
 $tec2 =$model->pegarespon();
?>
<div class="fieldBody">
    <form method="POST">
        <div class="row">
        ...
        <div class="row">
            <div class="col-lg-4">
                <?php echo formulario::select('dttie_resp_tec', $tec, 'Atribuído a:', @$campos['dttie_resp_tec']); ?>
            </div>
            <div class="col-lg-4">
                <?php
                /*
                ...
                $options = [
                    ...
                    'Cancelado' => 'Cancelado',
                    'Finalizado' => 'Finalizado',
                    'Não Visualizado' => 'Não Visualizado'
                ];
                echo formulario::select('status_sup', $options, 'Status');
                ?>
            </div>
            <div class="col-lg-4" style="text-align: right">
                <input class="btn btn-success" type="submit"  style="width: 300px" name="buscar" value="Buscar" />
            </div>           
        </div>
    </form>
    <br />
    ...
    if (!empty($_POST['id_sup'])) {
        ...
        if (!empty($_POST['dttie_resp_tec'])) {
            $resp_sup = " and resp_sup = " . $_POST['dttie_resp_tec'];
        }
        ...
        if (empty($_POST['id_sup'])) {
            if (!empty($_POST['status_sup'])) {
                ...
                $status_sup = " and `status_sup` in ('Aberto','Em Espera','Em Execução','Em Produção', 'Em Andamento', 'Não Visualizado', 'Atribuído')";
                //$status_sup = " and `status_sup` in ('Não Visualizado', 'Atribuído')";
            }
        }
        if (!empty($_POST['priori_sup'])) {
            $priori_sup = " and priori_sup like '%" . $_POST['priori_sup'] . "%'";
        }
        if (!empty($_POST['id_resp'])) {
            $id_resp = " and id_resp = " . $_POST['id_resp'];
        }

        if (tool::id_inst() == 124) {

            $tipo_sup = " and tipo_sup IN(76,78) ";
            $ordem = ' dt_prev_sup desc, dt_sup desc ';
            $and = NULL;
        } else {
            if (tool::id_nivel() == 4) {
                $sql = "SELECT * FROM `acesso_pessoa` WHERE `fk_id_gr` = 17";
                $query = pdoSis::getInstance()->query($sql);
                $array = $query->fetchAll(PDO::FETCH_ASSOC);

                $suporte = array_column($array, 'fk_id_pessoa');
                if (in_array(tool::id_pessoa(), $suporte)) {
                    $and = " and tipo_sup NOT IN (76) ";
                }
            }
            $ordem = ' dt_sup desc ';
        }
    ...
    if (!empty($array)) {
        ...
        $ti = sql::get('dttie_tipo_cadamp');
        foreach ($ti as $w) {
            $tipocad[$w['id_tipo_cadamp']] = $w['n_tipo_cadamp'];
        }
        if (tool::id_pessoa() == 1) {
            $novaPagina = 1;
        }
        foreach ($array as $k => $v) {
            ...
            @$array[$k]['tipo_cadamp'] = $tipocad[$v['tipo_cadamp']];
            @$array[$k]['n_resp2'] = $tec2[$v['resp_sup']];
