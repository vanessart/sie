    ...
    <div class="row">
        ...
        <div class="col-sm-3">
            <div class="fieldWhite text-center" style="padding: 20px">
                ...
                <br />
                <form target="_blank" action="<?php echo HOME_URI ?>/transp/alunoPlan" id="ex" method="POST">
                    <?php
                    foreach ($linha as $k => $v) {
                        ?>
                ...
                <br />
                <form target="_blank" action="<?php echo HOME_URI ?>/app/excel/doc/transpexcluido.php" id="ex" method="POST">
                    <?php
                    foreach ($linha as $k => $v) {
                        ?>
                        <input id="excel<?php echo @$v['id_li'] ?>" type="hidden" name="sel[]" value="" />                           
                    ...
                    <input type="hidden" name="relatorio" value="<?php echo $relatorio ?>" />
                    <input type="hidden" name="idinst" value="<?php echo $id_inst ?>" />
                    <button name="lista"  style="width: 80%" type="submit" class="art-button" id="btnla">
                        Movimentação Exclusão em Excel
                    </button>
                </form> 
                <br />
                <form target="_blank" action="<?php echo HOME_URI ?>/app/excel/doc/transpincluido.php" id="ex" method="POST">
                    <?php
                    foreach ($linha as $k => $v) {
                        ?>
                        <input id="excel<?php echo @$v['id_li'] ?>" type="hidden" name="sel[]" value="" />                           
                ...
                <br />
                <form target="_blank" action="<?php echo HOME_URI ?>/transp/frequenciapdfbranco" id="controleb" method="POST">
                    <?php
                    foreach ($linha as $k => $v) {
                        ?>
                    ...
                    <input type="hidden" name="idinst" value="<?php echo $id_inst ?>" />
                    <input type="hidden" name="relatorio" value="<?php echo $relatorio ?>" />
                    <button name="controleb" value="Controle Freq" onclick="listaaluno('controleb')" onmouseover="cb()" style="width: 80%" type="button" class="art-button" id="btncb">
