    ...
    <iframe style="width: 100%; height: 50vh; border: none" src="<?php echo HOME_URI ?>/gt/cadusersecre"></iframe>
        ...
        foreach ($fun as $k => $v) {
            ...
            if (in_array(@$v['id_pessoa'], $idAcesso)) {
                $fun[$k]['acesso'] = formulario::submit('Excluir Acesso', NULL, ['id_pessoa' => $v['id_pessoa'], 'id_inst' => $id_inst, 'escluirUser' => 1], NULL, NULL, NULL, ' btn btn-danger');
            } else {
            ...
            $fun[$k]['del'] = formulario::submit('Excluir Funcionário', NULL, ['fk_id_inst' => $id_inst, 'id_pessoa' => $v['id_pessoa'], 'excluirFunc' => 1]);
        }
        $form['array'] = $fun;
        $form['fields'] = [
            'Nome' => 'n_pessoa',
