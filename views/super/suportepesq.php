...
<div class="fieldBody">
    ...
    if (!empty($_POST['id_sup'])) {
        ...
        if (empty($_POST['id_sup'])) {
            if (!empty($_POST['status_sup'])) {
                ...
                $status_sup = " and `status_sup` in ('Aberto','Em Espera','Em Execução','Em Produção', 'Em Espera', 'Em Andamento')";
            }
        }
    ...
    $sql = "SELECT $fields FROM super_suporte_trab t "
            ...
            . "  " . @$status_sup . ' '
            . "  " . @$priori_sup . ' '
            . "order by dt_sup desc limit 0, 500 ";
    ...
    if (!empty($array)) {
        ...
        foreach ($array as $k => $v) {
            ...
            @$array[$k]['diag'] = NULL;
            $sql = "SELECT * FROM `super_suport_diag` "
            ...
            foreach ($diag1 as $dd) {
                @$array[$k]['diag'] .= $dd['lado'] . ': ' . $dd['descr'] . '<br />';
            }
            ...
            @$array[$k]['ac'] = formulario::submit($edit, NULL, ['id_sup' => $v['id_sup']], HOME_URI . '/super/suporte', @$novaPagina, NULL, $class);
