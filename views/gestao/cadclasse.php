...
$todosPeriodos = turma::periodoTodos($periodoLetivo);
?>
<div class="fieldBody">
    <div class="row">
        <div class="col-md-8" style="text-align: center; font-size: 18px">
            Cadastro de Classes (<?php echo implode(' - ', $todosPeriodos) ?>)
        </div>
        <div class="col-md-2">
            <input style="width: 100%" class="btn btn-info" type="submit" onclick=" $('#myModal').modal('show');" value=" Nova Classe" />
        </div>
        <div class="col-md-2">
            <form method="POST">
                <input type="hidden" name="periodoLetivo" value="<?php echo $periodoLetivo == 1 ? 2 : 1 ?>" />
                <button style="width: 100%" class="btn btn-<?php echo $periodoLetivo == 1 ? 'success' : 'warning' ?>">
                    <?php echo $periodoLetivo == 1 ? 'Ver Próximo Período' : 'Ver Período Atual' ?>
                </button>
            </form>
        </div>
    ...
    <div class="row">
        <div class="col-lg-12">
            <?php $model->listTurma(tool::id_inst(), $periodoLetivo); ?>
        </div>
    </div>
</div>
<?php
            ...
            <form method="POST">
                <div class="row">
                    ...
                    <div class="col-lg-3">
                        ...
                        <?php @formulario::select('1[fk_id_pl]', curso::periodList($ciclo['id_ciclo']), "periodo Letivo", @$turma['fk_id_pl'], NULL, NULL, ' required ') ?>
                    </div>
                    <div class="col-lg-3">
                        <?php
                        for ($c = 'A'; $c < 'Z'; $c++) {
...
if (!empty($_POST['clonar'])) {
    ...
    <div style="min-height: 80vh">
        ...
        <div class="row">
            <div class="col-lg-4" style="height: ">
                <?php
                echo formulario::select('id_tp_ens', $model->escola['segmentos'], 'Segmento: ', (empty($id_tp_ens)? @$_POST['id_tp_ens']:$id_tp_ens), 1, ['clonar' => 1,'id_turma' => @$_POST['id_turma']]);
                $hidden = ['clonar' => 1, 'id_tp_ens' => @$_POST['id_tp_ens'], 'id_turma' => @$_POST['id_turma']];
                ?>
                <br />
            </div>
            <div class="col-lg-4">
                <?php
                if (!empty(@$id_tp_ens)) {
                    formulario::selectDB('ge_cursos', 'id_curso', 'Curso: ', @$_POST['id_curso'], NULL, '1', $hidden, NULL, ['fk_id_tp_ens' => @$id_tp_ens]);
                    $hidden['id_curso'] = @$_POST['id_curso'];
                }
                ?>
            ...
            <div class="col-lg-4">
                <?php
                if (!empty($id_curso)) {
                    formulario::selectDB('ge_ciclos', 'id_ciclo', 'Ciclo: ', @$_POST['id_ciclo'], NULL, '1', $hidden, NULL, ['fk_id_curso' => $id_curso]);
                    $hidden['id_ciclo'] = @$_POST['id_ciclo'];
                }
                ?>
