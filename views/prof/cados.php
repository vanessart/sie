<?php
@$edit = $_POST['edit'];
funcionarios::autocompleteRm('prof', 1);
...
<div class="fieldBody">
    ...
    <div>
        ...
        <form method="POST">
            <div class="fieldWhite"  id="fm" style="display:  <?php echo!empty($edit) ? "" : 'none' ?>; width: 95%">
                <div class="row">
                    <div class="col-md-12">
                        <?php
                        $sql = "SELECT f.rm, n_pessoa FROM `ge_funcionario` f "
                                . "join pessoa p on p.id_pessoa = f.fk_id_pessoa "
                                . " WHERE `fk_id_inst` = " . tool::id_inst()
                                . " order by p.n_pessoa ";
                        $query = $model->db->query($sql);
                        $prof = $query->fetchAll();
                        foreach ($prof as $v) {
                            $profRm[$v['rm']] = $v['n_pessoa'];
                        }
                        
                        echo formulario::select('rm', $profRm, 'professor', @$_POST['rm'])
                        ?>
        ...
        <div class="row">
            <div class="col-md-12">
                <?php
                $sql = "select distinct nao_hac, p.n_pessoa, p.email, pe.rm, pe.id_pe, pe.disciplinas, hac_periodo, hac_dia from ge_prof_esc pe "
                ...
                $sqlkey = DB::sqlKey('ge_prof_esc', 'delete');
                foreach ($pf as $k => $v) {
                    $nd = NULL;
