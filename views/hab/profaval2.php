        ...
        <table class="table table-bordered table-responsive fieldBorder2" style="margin: 0 auto; width: 100%; background-color: white;">
            <thead>
                ...
                $sql = "SELECT * FROM `hab_aluno` "
                        . " WHERE `fk_id_pessoa` IN (" . implode(',', $id_aluno) . ")";
                ...
                for ($c = 1; $c <= count($alunos); $c++) {
                    ...
                    <tr style="background-color: #7E9AA7;" >
                        <td style="width: 100px ;" >
                            <?php
                            if (file_exists(ABSPATH . "/pub/fotos/" . $alunos[$c]['id_pessoa'] . ".jpg")) {
                                ?>
                                <ul class="imgExpande">
                                    <li><a target="_blank"><img src="<?php echo HOME_URI ?>/pub/fotos/<?php echo $alunos[$c]['id_pessoa'] ?>.jpg?ass=<?php echo uniqid() ?>" width="100" height="120" alt="foto"/></a></li>
                                </ul>
                                <?php
                            } else {
                                ?>
                        ...
                        <td colspan="5">
                            <div style="width: 200px">
                                ...
                                <?php echo tool::abrevia($alunos[$c]['n_pessoa'], 20) ?>
                                <br /><br />
                                RSE/ID
                                <?php echo $alunos[$c]['id_pessoa'] ?>
                                <br />
                                Situação: 
                                <?php echo $alunos[$c]['situacao'] ?>
                            </div>
                        </td>
                </thead>
                <tbody>
                                    ...
                                    if (!empty($v['hab' . $i])) {
                                        ...
                                        <tr>
                                            <td style="text-align: center">
                                                <input type="hidden" name="<?php echo $alunos[$c]['id_pessoa'] .'_'. $v['id_competencia_opcoes'] ?>[id_aluno]" value="<?php echo @$habAluno[$alunos[$c]['id_pessoa']][$v['id_competencia_opcoes']]['id_aluno'] ?>" />
                                                <input type="hidden" name="<?php echo $alunos[$c]['id_pessoa'] .'_'. $v['id_competencia_opcoes'] ?>[fk_id_pessoa]" value="<?php echo $alunos[$c]['id_pessoa'] ?>" />
                                                <input type="hidden" name="<?php echo $alunos[$c]['id_pessoa'] .'_'. $v['id_competencia_opcoes'] ?>[fk_id_competencia_opcoes]" value="<?php echo $v['id_competencia_opcoes'] ?>" />
                                                <input <?php echo @$habAluno[$alunos[$c]['id_pessoa']][$v['id_competencia_opcoes']]['hab'.$i]==3?'checked':'' ?> style="cursor:pointer;" type="radio" name="<?php echo $alunos[$c]['id_pessoa'] .'_'. $v['id_competencia_opcoes'] ?>[<?php echo 'hab' . $i ?>]" value="1" />
                                            </td>
                                            <td style="text-align: center">
                                                <input  <?php echo @$habAluno[$alunos[$c]['id_pessoa']][$v['id_competencia_opcoes']]['hab'.$i]==2?'checked':'' ?> style="cursor:pointer;" type="radio" name="<?php echo $alunos[$c]['id_pessoa'] .'_'. $v['id_competencia_opcoes'] ?>[<?php echo 'hab' . $i ?>]" value="0.5" />
                                            </td>
                                            <td style="text-align: center">

                                                <input <?php echo @$habAluno[$alunos[$c]['id_pessoa']][$v['id_competencia_opcoes']]['hab'.$i]<=1?'checked':'' ?> style="cursor:pointer;" type="radio" name="<?php echo $alunos[$c]['id_pessoa'] .'_'. $v['id_competencia_opcoes'] ?>[<?php echo 'hab' . $i ?>]" value="0" />
                                            </td>
                                            <td>
                                                <?php echo $v['hab' . $i] ?> 
                                            </td>
