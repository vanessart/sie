<?php
$ciclos = [25, 26, 27, 28, 29, 30, 35, 36, 31,37];
$result = ensino::qa($ciclos);
$cic = sql::get('ge_ciclos');
foreach ($cic as $v) {
    $n_ciclos[$v['id_ciclo']] = $v['n_ciclo'];
}
$ajustealuno = $result['AlunoCicloPeriodo'][40]['T'][37];
$result['AlunoCicloPeriodo'][40]['M'][37] = $ajustealuno;
$ajusteclasse = $result['turmas'][40]['T'][37];
$result['turmas'][40]['M'][37] = $ajusteclasse;
asort($result['escola']);
?>
<hr>
<div class="fieldBody">
    <div style="text-align: center">
        NÚMERO DE CLASSES E ALUNOS - EJA 
    </div>
    <div  style="text-align: center">
        Atualizado em
        <?php
    ...
    if (empty($pdf)) {
        ?>
        <div>
            <form target="_blank" action="<?php echo HOME_URI ?>/info/qaejaPdf" style="text-align: right; width: 100%" method="POST">
