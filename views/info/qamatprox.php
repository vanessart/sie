...
<div class="fieldBody">
    <div style="text-align: center">
        NÚMERO DE CLASSES E ALUNOS - ENSINO INFANTIL - MATERNAL 
    </div>
    <div  style="text-align: center">
        Atualizado em
        <?php
    ...
    if (empty($pdf)) {
        ?>
        <div>
            <form target="_blank" action="<?php echo HOME_URI ?>/info/qamatPdfprox" style="text-align: right; width: 100%" method="POST">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    foreach ($ciclos as $ci) {
                        ?>
                        <td>
                            <?php echo (@$result['totalAlunoCiclo'] == 0 OR @$result['totalTurmas'][$ci] == 0) ? '-' : round(@$result['totalAlunoCiclo'][$ci] / @$result['totalTurmas'][$ci], 1) ?>
                        </td>
                        <?php
                    }
                    ?>
        ...
        <table class="table table-bordered">
            <thead>
                <tr style="font-weight: bold">
                    <td style="background-color:  #3c3c3c; color: white">
                        nº
                    </td>
                    <td style="background-color:  #3c3c3c; color: white">
                        Escola
            ...
            <tr>
                ...
                foreach (['M'] as $p) {
                    foreach ($ciclos as $ci) {
                        ...
                        <td>
                            <?php
                            echo ($tturma[$p][$ci] == 0) ? '-': round($talunos[$p][$ci] / $tturma[$p][$ci], 1);
                            ?>
                        </td>
                        <?php
                    }
