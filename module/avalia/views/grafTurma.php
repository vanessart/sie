...
<div class="fieldTop">
    <div class="row">
        <div class="col-md-10">
            Gráficos por Classe - <?php echo $unidLetiva_ ?>º <?php echo $curso['un_letiva'] ?>
        </div>
        <div class="col-md-2">
            <?php
            if (empty($_REQUEST['i'])) {
                ?>
                <a class="btn btn-success" href="#" onclick="window.open('<?php echo HOME_URI ?>/avalia/grafTurma?i=1&unidLetiva=<?php echo $unidLetiva_ ?>', 'Pagina', 'STATUS=NO, TOOLBAR=NO, LOCATION=NO, DIRECTORIES=NO, RESISABLE=NO, SCROLLBARS=YES, TOP=10, LEFT=10, WIDTH=770, HEIGHT=400');">Imprimir</a>
                <?php
            }
            ?>
        </div>
...
if (!empty($id_inst)) {
    ...
    if (!empty($conta)) {
        foreach ($conta as $k => $v) {
            foreach ($v as $kk => $vv) {
                $result[$turmas_[$k]][$disc[$kk]] = str_replace(',', '.', $notas[$k][$kk]) / $vv;
            }
        }
