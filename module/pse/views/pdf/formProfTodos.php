...
$alunos = $model->getAlunos($id_turma, $resp, 2);
ob_clean();
ob_start();
//$pdf = new pdf();
foreach ($alunos as $k => $v) {
    ...
    $prof_responde = sql::get('audicao_form_resposta', 'fk_id_pessoa_responde', ['fk_id_form' => '2', 'fk_id_pessoa' => $v['id_pessoa'], 'fk_id_campanha' => $id_campanha  ], 'fetch');
    if (!empty($prof_responde)) {
       $prof = $model->getProf($prof_responde['fk_id_pessoa_responde']); 
    }
    $dados_aluno = $model->getPessoa($v['id_pessoa']);
    $aluno_turma = $model->alunoSala($v['id_pessoa']);
    ...
    <?php $model->getViewPDF($form,$v['id_pessoa'],2,$resp); ?>
    <div class="row">
