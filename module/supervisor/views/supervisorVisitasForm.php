    ...
    <form id="atr" method="POST">
        <div class="row">
            <?php if ($eh_coordencacao): ?>
                <div class="col-6">
                    <?= formErp::select('1[fk_id_pessoa]', $aPessoas, 'Supervisor', $fk_id_pessoa); ?>
                </div>
            <?php else: ?>
                <div class="col-md-6">
                        Supervisor: <?= @$aPessoas[$result['fk_id_pessoa']]?>
                </div>
            <?php endif ?>
        </div>
        <div class="row mt-3">
                ...
                <div class="input-group mb-6">
                    ...
                    <select name="instituicao" class="form-select" aria-label="Default select example">
                        ...
                        <optgroup label="Rede">
                            ...
                            <option value="<?=$k?>" <?= $result['fk_id_inst'] == $k ? 'selected' : ''?>><?=$v?></option>
                            <?php endforeach ?>
                        </optgroup>
                        <?php endif ?>
            ...
            <div class="col-4">
                <select name="1[fk_id_curso]" class="form-select">
                    <option value="">Selecione</option>
                </select>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-3">
                <div class="input-group mb-3">
                    ...
                    <input type="date" name="1[data_visita]" class="form-control" value="<?= $result['data_visita'] ?>" required/>
                </div>
            </div>
            <div class="col-3">
                <div class="input-group mb-3">
                    <span class="input-group-text" id="basic-addon3">Horário Inicial</span>
                    <input type="time" name="1[periodo_inicial]" min="09:00" max="18:00" class="form-control" value="<?= $result['periodo_inicial'] ?>" required />
                </div>
            </div>
            <div class="col-3">
                <div class="input-group mb-3">
                    <span class="input-group-text" id="basic-addon3">Horário Final</span>
                    <input type="time" name="1[periodo_final]" min="09:00" max="18:00" class="form-control" value="<?= $result['periodo_final'] ?>" required />
                </div>
            </div>
        </div>
        <br><br><br>
        <div class="col" style="text-align:center">
            <?php
                $arrFields = [
                    '1[id_visita]' => $result['id_visita'],
                    '1[fk_id_inst]' => $result['fk_id_inst'],
                    '1[rede]' => $result['rede'],
                    '1[at_visita]' => $result['at_visita'],
                    'back' => '1',
                ];
                if (!$eh_coordencacao) {
                    $arrFields['1[fk_id_pessoa]'] = $fk_id_pessoa;
                }
                echo formErp::hidden($arrFields);
            ?>
            <?= formErp::hiddenToken('vis_visita','ireplace',null,null,1) ?>
