    ...
    <div class="geral">
        ...
        if (!empty($tpd)) {
            ...
            foreach ($tpd as $key => $value) {
                ...
                if (!is_numeric($key) && in_array($value['id_curso'], [1, 5, 9])) {
                                  $temTurma = 1
      ?>
                    <form action="<?= HOME_URI . '/profe/lancNotaSet' ?>" method="post" id="form_ids-<?= $value['id_turma'] ?>-<?= $value['id_disc'] ?>">
                        ...
                        <div style="text-align: center">
                            <button class="btn btn-success" style="width: 650px">
                                <?= $value['nome_turma'] ?> - 
                                <?= $value['nome_disc'] ?> - 
                                <?= $value['escola'] ?> - 
                            </button>
