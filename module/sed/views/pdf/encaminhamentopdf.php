...
if (!empty($_POST['imprimir'])) {
    ...
    $dados = encaminhamento($destino, $idturma);
} else {
...
foreach ($dados as $v) {
    ...
    <div style="text-align: left; font-size: 8pt">
        ...
        d. Xerox do comprovante de endereço atualizado, em nome do pai, mãe ou responsável legal <?php echo $sexo = ($v['sexo'] == "M") ? ' pelo aluno, ' : ' pela aluna, ' ?> morador de <?= CLI_CIDADE ?>;<br />
    ...
    <br />
    <?php
    $end = pegaenderecoescola($v['escola_destino']);
    ?>
    <br />
...
function encaminhamento($idesc, $turma) {
...
function pegaenderecoescola($idinst) {

    $sql = "SELECT p.id_predio, p.logradouro, p.num, p.bairro, p.tel1 FROM instancia_predio ip"
            . " JOIN predio p ON p.id_predio = ip.fk_id_predio"
            . " WHERE ip.fk_id_inst = '" . $idinst . "'";

    $query = pdoSis::getInstance()->query($sql);
    $ed = $query->fetch(PDO::FETCH_ASSOC);

    $endereco['end'] = $ed['logradouro'] . ' nº ' . $ed['num'] . ' - ' . $ed['bairro'];
    $endereco['tel'] = $ed['tel1'];
