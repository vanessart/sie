...
<div class="body">
    ...
    <div class="row">
        ...
        <div class="col-md-2">
            <?= formErp::selectDB('cadampe_status', 'fk_id_status', 'Situação', $id_status, 1, $hidden) ?>
        </div>
        <div class="col-md-6">
...
<script>
    ...
    function editAdmin(id,id_status,n_status){
        document.getElementById("id_cadampe_pedido").value = id;
        var titulo= document.getElementById('myModalLabel');
        if (id_status==3){
            titulo.innerHTML  = "Protocolo "+id+" <label class='modal_green'>"+n_status+"</label";
        ...
        document.getElementById("form").action = '<?= HOME_URI ?>/cadampe/protocoloEdit';
        document.getElementById("form").submit();
        $('#myModal').modal('show');
        $('.form-class').val('');
    }
