...
if (!defined('ABSPATH'))
    exit;
$sql = "SELECT `id_cate`, `n_cate` FROM `inscr_categoria` ORDER BY `inscr_categoria`.`n_cate` ASC ";
$query = pdoSis::getInstance()->query($sql);
$cate = $query->fetchAll(PDO::FETCH_ASSOC);
##################            
?>
  <pre>   
    <?php
...
exit();
$array = $model->classifica($id_cate);
@$nomearquivo = 'deferido_' . preg_replace(array("/(á|à|ã|â|ä)/", "/(Á|À|Ã|Â|Ä)/", "/(é|è|ê|ë)/", "/(É|È|Ê|Ë)/", "/(í|ì|î|ï)/", "/(Í|Ì|Î|Ï)/", "/(ó|ò|õ|ô|ö)/", "/(Ó|Ò|Õ|Ô|Ö)/", "/(ú|ù|û|ü)/", "/(Ú|Ù|Û|Ü)/", "/(ñ)/", "/(Ñ)/"), explode(" ", "a A e E i I o O u U n N"), end(explode(' ', $n_cate)));
;
