...
$serial = filter_input(INPUT_POST, 'id_serial', FILTER_SANITIZE_STRING);
$modelo = filter_input(INPUT_POST, 'id_equipamento', FILTER_SANITIZE_NUMBER_INT);
$destino = filter_input(INPUT_POST, 'destino', FILTER_SANITIZE_NUMBER_INT);
$situacao = filter_input(INPUT_POST, 'id_situacao', FILTER_SANITIZE_STRING);
...
$equipamentoGet = $equipamento = $model->equipamentoSelect();
// unset($sitList[0]);
// unset($sitList[3]);
...
<div class="body">
    <div class="fieldTop">
        <?php
        if (empty($resumo)) {
            echo 'Consulta Geral de Equipamentos';
        } else {
            echo 'Resumo - Equipamentos';
        }
        ?>
                ...
                <div class="col">
                    <div class="input-group">
                        <div class="input-group-prepend">
                            <?= formErp::select('id_serial', $objetoList, 'Nº de Série', $serial) ?> 
                        </div>
                    </div>
                </div>
                <div class="col">
                     <?= formErp::select('id_equipamento', $equipamentoGet, 'Modelo/Lote', $modelo) ?>
                </div>
                <div class="col">
                    <?= formErp::selectDB('recurso_situacao', 'id_situacao', 'Situação', $situacao) ?>
                </div>
            </div>
            <br />
            <div class="row">
