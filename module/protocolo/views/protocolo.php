...
if ($id_protocolo) {
    ...
    if (!empty($status['n_pessoa'])) {
       $hidden['pessoa_status'] = $status['n_pessoa']; 
    }
    if (!empty($status['n_status'])) {
       $hidden['n_status'] = $status['n_status']; 
    }
    if (!empty($status['dt_status'])) {
       $hidden['dt_status'] = $status['dt_status']; 
    }
} else {
    $ativo = null;
}?>
<div class="body">
    ...
    $abas[3] = ['nome' => "Histórico", 'ativo' => $ativo, 'hidden' => $hidden];
    $aba = report::abas($abas);
    require ABSPATH . "/module/protocolo/views/_protocolo/$aba.php"
    ?>
</div>