...
$sql = "SELECT   i.n_inst, ga.ciclos, ga.fk_id_disc, gr.* FROM global_respostas gr "
...
// . " and ciclos = $id_ciclo "
 . " and fk_id_disc in (6,9) "
 . " and ciclos in (1,2,3,4,5,6,7,8,9) "
 . " order by n_inst, fk_id_disc, ciclos";
$query = pdoSis::getInstance()->query($sql);
$array = $query->fetchAll(PDO::FETCH_ASSOC);
...
foreach ($array as $v) {
    foreach ($v as $kk => $vv) {
        if (substr($kk, 0, 1) == 'q' && !empty($vv) && $kk != 'quest') {
            ...
            if ($vv == 1) {
                @$dado[$v['n_inst']][$v['fk_id_disc']][$v['ciclos']][$num] ++;
            }
            @$total[$v['n_inst']][$v['fk_id_disc']][$v['ciclos']][$num] ++;
        }
    }
}
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . '1', 'Hab');

$objPHPExcel->getActiveSheet()->getColumnDimension('E')->setAutoSize(100);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . '1', 'Hab. Desc');

$objPHPExcel->getActiveSheet()->getColumnDimension('F')->setAutoSize(50);
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . '1', '%');
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H' . '1', 'QTD_ANO_ESCOLA_DISC');
    ...
    foreach ($v as $kk => $vv) {
        foreach ($vv as $kkk => $vvv) {
            foreach ($vvv as $kkkk => $vvvv) {
                ...
                @$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $cont, $hab[$kkk][$kk][$kkkk]);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . $cont, (($vvvv / $totalEsc) * 100));
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $cont, $vvvv);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H' . $cont, $totalEsc);
                $cont++;
            }
        }
    }
...
header('Content-Disposition: attachment;filename="' . 'arquivo' . '_' . date("Y_m_d") . '.xls"');
