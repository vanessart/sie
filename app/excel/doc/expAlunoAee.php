...
$colunas = 'id_pessoa, ra, ra_dig, ra_uf, n_pessoa, dt_nasc, sexo, id_turma, n_curso, n_turma, id_inst, n_inst, deficiencia as APD ';
...
@$nomearquivo = 'Alunos_AEE_Regular' . $id_curso;
...
if (!empty($array)) {
    ...
    foreach ($array as $k => $v) {
        ...
        $wsql = "SELECT ta.fk_id_pessoa, t.id_turma, t.n_turma, i.id_inst, i.n_inst FROM ge_turma_aluno ta"
                . " JOIN ge_turmas t ON t.id_turma = ta.fk_id_turma"
                . " JOIN ge_periodo_letivo pl ON pl.id_pl = t.fk_id_pl"
                . " JOIN instancia i ON i.id_inst = ta.fk_id_inst"
                . " JOIN ge_ciclos ci ON ci.id_ciclo = t.fk_id_ciclo"
                . " JOIN ge_cursos c ON c.id_curso = ci.fk_id_curso "
                . " WHERE pl.at_pl = 1 AND ta.situacao = '" . 'Frequente' . "'"
                . " AND ta.fk_id_pessoa = " . $v['id_pessoa'] . " AND c.id_curso != 10";

        $query = pdoSis::getInstance()->query($wsql);
        $array2 = $query->fetch(PDO::FETCH_ASSOC);
       
        if (!empty($array2)) {
            $array[$k]['id_turma_Regular'] = $array2['id_turma'];
            $array[$k]['n_turma_Regular'] = $array2['n_turma'];
            $array[$k]['id_inst_Regular'] = $array2['id_inst'];
            $array[$k]['n_inst_Regular'] = $array2['n_inst'];
        }
    }
