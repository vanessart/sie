...
$sql = @$_POST['sql'];
if (!empty($sql) && $_POST['tokenSql'] == $tokenSql) {
    $sql = str_replace(';', '', $sql);
    @$nomearquivo = !empty($_POST['nomearquivo']) ? $_POST['nomearquivo'] : 'Arquivo';


    if ((strcasecmp(substr($sql, 0, 6), 'select') == 0) || !empty($_POST['catatau999'])) {
