...
if (empty($_POST['mes'])) {
    ...
    $sql = " SELECT "
            . " `fk_id_cad`, cad_pmb, SUM(`horas`) as total "
            . " FROM cadam_freq f "
    ...
    $sql = " SELECT "
            . " `fk_id_cad`, cad_pmb, SUM(`horas`) as total "
            . " FROM cadam_freq_tea ft "
            . " JOIN cadam_cadastro ct on ct.id_cad = ft.fk_id_cad "
            . " WHERE mes = " . $mes
            . " AND ano = " . $ano
            . " GROUP BY fk_id_cad ";
...
// Set document properties
        ...
        foreach ($tt as $v) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $c, $v['total'] + $v['total2']);
            $c++;
        }
...
// Rename worksheet
        $objPHPExcel->getActiveSheet()->setTitle('Relatorio_horas_' . $mes);
...
// Redirect output to a client’s web browser (Excel5)
        ...
        header('Content-Disposition: attachment;filename="' . 'Relatorio_horas_' . $mes . '.xls"');
